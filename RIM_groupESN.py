...
class GroupESNCell(nn.Module):
	...
	def __init__(
		...
		super(GroupESNCell, self).__init__()
		...
		self.leakies = torch.FloatTensor(self.leakies).to(self.device)
		...
		self.kernels = nn.Parameter(self.kernels, requires_grad=False)
		...
		for i in range(self.num_esns):
			...
			self.recurrent_kernels.append(recurrent_kernel)
		...
		self.recurrent_kernels = nn.Parameter(self.recurrent_kernels, requires_grad=False)
		self.biases = torch.stack(self.biases)
		self.biases = nn.Parameter(self.biases, requires_grad=False)
	...
	def forward(self, xt: torch.Tensor, h_prev: torch.Tensor):
		...
		leaky_output = h_prev * (torch.ones(self.num_esns).to(self.device) - self.leakies).view(-1, 1, 1) + output * self.leakies.view(-1, 1, 1)

		#return leaky_output, leaky_output
		print(f'ESN output.shape: {leaky_output.shape}')
		return leaky_output
...
class GroupLSTMCell(nn.Module):
	...
	def forward(self, x, hid_state):
		...
		print(f'LSTM output.shape: {h_t.shape}')
		return h_t, c_t
...
class GroupGRUCell(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        print(f'GRU output.shape: {hy.shape}')
        return hy
...
class RIMCell(nn.Module):
	def __init__(self, 
		...
		if self.rnn_cell == 'GRU':
			...
			#self.rnn = nn.ModuleList([ReservoirCell(input_value_size, units=hidden_size) for _ in range(num_units)])
			self.rnn = GroupESNCell(num_units, input_value_size, [hidden_size]*num_units, device=device)
			self.query = GroupLinearLayer(hidden_size, input_key_size * num_input_heads, self.num_units)
		else:
	...
	def forward(self, x, hs, cs = None):
		...
		print(f'x.shape before input_attention_mask: {x.shape}')
